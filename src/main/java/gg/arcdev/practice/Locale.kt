...
enum class Locale(
    ...
    fun format(vararg objects: Any): String {
        return MessageFormat(
            ChatColor.translateAlternateColorCodes(
                '&',
                Main.getInstance().mainConfig.getString(path)
            )
        ).format(objects)
    }

    fun formatLines(vararg objects: Any): MutableList<String> {
        val lines: MutableList<String> = ArrayList()

        if (Main.getInstance().mainConfig.get(path) is String) {
            lines.add(
                MessageFormat(
                    ChatColor.translateAlternateColorCodes(
                        '&',
            ...
            for (string in Main.getInstance().mainConfig.getStringList(path)) {
                lines.add(
                    MessageFormat(ChatColor.translateAlternateColorCodes('&', string))
                        .format(objects)
                )
            }
        }
